...
/* the upgrade for rev 3 boards also works fine on rev 2 and rev 4 */
#define INCLUDE_BIOS_10204
#define INCLUDE_BIOS_30003
#define INCLUDE_BIOS_40001
...
#endif
#ifdef INCLUDE_BIOS_30003
#include "upgrade30003.h"
#endif
#ifdef INCLUDE_BIOS_40001
#include "upgrade40001.h"
#endif
...
void Reattach();
void bye(char* msg);
void SendFile(int flen, uchar *fptr, int curbase, int base, int run);
int  DoFile(uchar *fdata, int base, int run, int flen, int skip, bool builtin);
...
int HandleTransfer(uchar *fdata, int base, int run, int flen, int skip, bool part2of6mb);
void CatVal(char *szOut, int nBufLen, int nVal, int nRow);
bool DetermineFileInfo(bool bMute, uchar *fdata, int *base, int *run, int *flen, int *skip);
...
int main(int argc, char* argv[])
{
	...
	if ((argc<2) || ((argc>1)&&(strchr(argv[1],'?')))) {
		bye("Usage: jcp [-k <0|1|2>] | [-rewfnbocdgpsuxt] file [$base]\n"	\
			...
			"  -t ($start) = override the start/run address\n" \
	...
	while (argv[nArg][0] == '-') {
		...
		while ((argv[nArg][nPos]) && (0 == fExitLoop)) {
			switch (tolower(argv[nArg][nPos])) {
				...
				case 't':
					g_OptOverrideRun=true;
					if (nArg+1 < argc) {
						nArg++;
						run = ParseAddress(argv[nArg]);
						nFileArg++;
						nBaseArg++;
						fExitLoop=1;
					} else {
						bye("-t option requires the run address to follow");
					}
					break;
	...
	DetermineFileInfo(false, fdata, &base, &run, &flen, &skip);
	...
	if (nCartBank == -1) {
		if (!g_OptOnlyBoot) {
			...
			g_OptNoBoot=true;
			g_OptConsole=false;
			nUsed=HandleTransfer(fdata, base, 0, flen, skip, false);
			WaitForBothBuffers();
			...
			HandleTransfer(fdata+nUsed, 0x800000, run, flen-nUsed, 0, true);
			WaitForBothBuffers();
		}
		...
		g_OptOnlyBoot=true;
		nCartBank=-1;
		DoFile(fdata, base, run, 0, 0, true);
	} else {
		HandleTransfer(fdata, base, run, flen, skip, false);
	}
...
int HandleTransfer(uchar *fdata, int base, int run, int flen, int skip, bool part2of6mb) {
	...
	return DoFile(fdata, base, run, flen, skip, part2of6mb);
}
...
void DoFlash(int nLen) {
	...
	DoFile((uchar*)FLASHSTUB, 0x4100, 0x4100, SIZE_OF_FLASHSTUB, 168, true);
...
void DoDump(char *pszName) {
	...
	DoFile((uchar *)romdump6, 0x10000, 0x10000, SIZE_OF_ROMDUMP6, 168, true);
...
void DoE2pDump(char *pszName) {
	...
	DoFile((uchar*)e2pget, 0x4000, 0x4000, SIZE_OF_E2PGET, 168, true);
...
void DoE2pWrite(char *pszName) {
	...
	DoFile((uchar*)e2pput, 0x4000, 0x4000, SIZE_OF_E2PPUT, 168, true);
...
//			printf("Polled 0x%08x\n", poll);
	...
	g_OptConsole=true;
	g_OptSilentConsole=true;
	DoFile((uchar*)DUMPVER, 0x5000, 0x5000, SIZE_OF_DUMPVER, 168, true);
	printf("\n");
}
...
void DoBiosUpdate() {
	...
	if ((!gotupgrade1) && (!gotupgrade3) && (!gotupgrade4)) {
	...
	if (g_OptOverrideFlash) {
		...
		g_OptConsole=false;
		g_skipwait=true;
		DoFile((uchar*)readver, 0x5000, 0x5000, SIZE_OF_READVER, 168, true);
		g_skipwait=false;
		Sleep(500);
	...
	if (((currentRev == 2)||(currentRev == 3)) && (!gotupgrade3)) {
		bye("\n\nBIOS Upgrade for Rev 2, 3 or 4 boards not included in this version of JCP\n");
	}
	if ((currentRev == 4) && (!gotupgrade4)) {
		bye("\n\nBIOS Upgrade for Rev 5 boards not included in this version of JCP\n");
	}

	if ((currentRev < 1) || (currentRev > 4)) {
		printf("\n\nCurrent rev is '%02x'\n", currentRev);
	...
	if (currentRev == 1) {
		...
		DoFile((uchar*)Upgrade10204, 0x80000, 0x80000, SIZE_OF_UPGRADE10204, 168, true);
#endif
	} else if (currentRev <= 3) {
		// 2 and 3
#ifdef INCLUDE_BIOS_30003
		// Major - SerBuf[6]
		// Minor - SerBuf[5]
		// Rev   - SerBuf[4]
		if (!bForce) {
			if ((SerBuf[6] == 3) && (SerBuf[5] == 0) && (SerBuf[4] == 3)) {
				bye("Board is already on revision 3.00.03 - upgrade not required (-fu to force).\n");
			}
		}

		printf("\n\nGoing to upgrade Rev %d board to Rev 3 BIOS 3.00.03 (okay for Rev2 and Rev4)\n\n", currentRev);
		DoFile((uchar*)upgrayyd30003, 0x80000, 0x80000, SIZE_OF_UPGRAYYD30003, 168, true);
#endif
	} else {
		// 4 (Rev5+ boards)
#ifdef INCLUDE_BIOS_40001
		if (!bForce) {
			if ((SerBuf[6] == 4) && (SerBuf[5] == 0) && (SerBuf[4] == 1)) {
				bye("Board is already on revision 4.00.01 - upgrade not required (-fu to force).\n");
			}
		}

		printf("\n\nGoing to upgrade Rev 5 board to BIOS 4.00.01\n\n");
		DoFile((uchar*)upgrade40001, 0x80000, 0x80000, SIZE_OF_UPGRADE40001, 168, true);
#endif
	}
}
...
int DoFile(uchar *fdata, int base, int run, int flen, int skip, bool builtin) {
	...
	if (!g_OptOnlyBoot) {
		if (!builtin) {
			...
			DetermineFileInfo(true, fdata, &base, &run, &flen, &skip);
	...
	if (!g_OptOnlyConsole) {
		...
		nRet=flen+skip;
		SendFile(flen, fptr, curbase, base, run);
	}
...
static bool IsRomHeader(const uchar *fdata, int flen, int offset, int *run)
{
	int start;

	// Verify the file is big enough to contain a ROM header:
	if (flen <= (0x2000 + offset)) {
		return false;
	}
...
bool DetermineFileInfo(bool bMute, uchar *fdata, int *base, int *run, int *flen, int *skip) {
	...
	// Check file header
	if (IsRomHeader(fdata, *flen, 0, run)) {
		if (!bMute) {
			if ( (g_OptVerbose) || (!g_OptSilentConsole) ) {
				printf("Cart ROM:  ");
			}
		...
		*skip = 0x2000;
	} else if (IsRomHeader(fdata, *flen, 0x200, run)) {
		if (!bMute) {
			if ( (g_OptVerbose) || (!g_OptSilentConsole) ) {
				printf("Cart ROM + 512:  ");
			}
		...
		*skip = 0xa8;
		*base = ENBIGEND(fdata + 0x28);
		if (!g_OptOverrideRun) *run = ENBIGEND(fdata + 0x24); // these files can have different run and base addresses.
	...
	if (!bMute) {
		if ( (g_OptVerbose) || (!g_OptSilentConsole) ) {
			printf("Skip %d bytes, base addr is $%X, run addr is $%X, length is %d bytes\n", *skip, *base, *run, *flen-*skip);
		}
	}
